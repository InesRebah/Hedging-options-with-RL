    ...
    def train(self, episodes, savetag=""):
        ...
        for ep in range(episodes):
            ...
            if ep % 1000 == 0 and ep > 0:
                ...
                self.actor.save_weights(f"model/gamma/ddpg_actor{tag}{ckpt}.weights.h5")
                self.critic_q_ex.save_weights(f"model/gamma/ddpg_critic_q_ex{tag}{ckpt}.weights.h5")
                self.critic_q_ex2.save_weights(f"model/gamma/ddpg_critic_q_ex2{tag}{ckpt}.weights.h5")

        tag = f"{savetag}" if savetag else ""
        self.actor.save_weights(f"model/gamma/ddpg_actor{tag}.weights.h5")
        self.critic_q_ex.save_weights(f"model/gamma/ddpg_critic_q_ex{tag}.weights.h5")
        self.critic_q_ex2.save_weights(f"model/gamma/ddpg_critic_q_ex2{tag}.weights.h5")

        return history

    def load(self, tag=""):
        suffix = f"{tag}" if tag else ""
        self.actor.load_weights(f"model/gamma/ddpg_actor{suffix}.weights.h5")
        self.actor_target.load_weights(f"model/gamma/ddpg_actor{suffix}.weights.h5")

        self.critic_q_ex.load_weights(f"model/gamma/ddpg_critic_q_ex{suffix}.weights.h5")
        self.critic_q_ex_target.load_weights(f"model/gamma/ddpg_critic_q_ex{suffix}.weights.h5")

        self.critic_q_ex2.load_weights(f"model/gamma/ddpg_critic_q_ex2{suffix}.weights.h5")
        self.critic_q_ex2_target.load_weights(f"model/gamma/ddpg_critic_q_ex2{suffix}.weights.h5")
...
if __name__ == "__main__":
    ...
    for freq_name, freq_val in FREQS.items():
        ...
        env = TradingEnv(
            ...
            gamma_flag=True #lambda = 0.01
        )

        ddpg = DDPG(env)
        hist = ddpg.train(TRAIN_EPISODES, savetag=f"gamma_{freq_name}")

        if hasattr(ddpg, "savehistory"):
            ddpg.savehistory(hist, f"ddpg_gamma_{freq_name}.csv")

    print("\nGamma training completed.")