...
import numpy as np
from utils import get_sim_path, get_sim_path_sabr, get_sim_path_fx, get_sim_path_commodity
...
class TradingEnv(gym.Env):
    ...
    def _generate_paths(self, model_params, seed=None):
        ...
        if self.asset_class == "fx":
            self.path, self.option_price_path, self.delta_path = get_sim_path_fx(
                M=self.init_ttm,
            ...
            self.path, self.option_price_path, self.delta_path = get_sim_path_commodity(
                M=self.init_ttm,
                freq=self.trade_freq,
                np_seed=seed,
                num_sim=self.num_sim,
    ...
    def step_profit_loss(self, action):
        ...
        position = float(np.asarray(action).reshape(-1)[0]) #position = action
        ttm = self.ttm_array[self.t]
